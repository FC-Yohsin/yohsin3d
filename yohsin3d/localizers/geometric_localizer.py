...
class GeometricLocalizer(BaseLocalizer):
    def __init__(self):
        super().__init__()
        self.global_static_objects_right = [VisibleObjects.F1R, VisibleObjects.G1R, VisibleObjects.G2R, VisibleObjects.F2R]
        self.global_static_objects_left = [VisibleObjects.F1L, VisibleObjects.G1L, VisibleObjects.G2L, VisibleObjects.F2L]

    
    def _get_straight_distance(self, distance, height):
        return math.sqrt((distance**2) - (height**2))

    def _search_two_visible_objects(self) -> tuple[VisibleObjects, VisibleObjects]:
        visible_objects = [visible_object for visible_object in self.world_model.simple_vision_objects.keys() if self.world_model.simple_vision_objects[visible_object] and visible_object != VisibleObjects.BALL]
        if len(visible_objects) >= 2:
            return (visible_objects[0], visible_objects[1])
        else:
            return None
    
    def _two_flag_position_localize(self, objects:tuple[VisibleObjects,VisibleObjects]) -> tuple[float, float, float]:
    ...
    def _one_flag_orientation_localize(self, object: VisibleObjects) -> float:
        ...
        base = abs((globalPositionOfWorldObject[base_axis])-(currentPosition[base_axis]))
        angle = base/hyp
