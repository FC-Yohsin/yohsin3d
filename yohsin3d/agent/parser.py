...
class Parser:
    ...
    def __parse_accelerometer(self, string):
        ...
        if (rateX != rateX or (abs(rateX) > spuriousThreshold)):
            rateX = 0

        if (rateY != rateY or (abs(rateY) > spuriousThreshold)):
            rateY = 0

        if (rateZ != rateZ or (abs(rateZ) > spuriousThreshold)):
            rateZ = 0
    ...
    def __parse_FRP(self, str):
        ...
        force_resistance_perceptors[name] = [(centreX, centreY, centreZ),(forceX, forceY, forceZ)]
        return True
    ...
    def __parse_hear(self: 'Parser', string):
        ...
        if len(tokens) == 5:
            team = tokens[i]
            i+=1
            if team != self.world_model.team_name:
                self.world_model.opponent_team_name = team
                return True

        hearTime = float(tokens[i])
        ...
        deltaServerToGameTime = self.world_model.get_time() - self.world_model.get_gametime()
        ...
        if proccess_success:
            ...
            self.world_model.teamMateLastHeardTime[playerNum - 1] = self.world_model.get_time()
            self.world_model.fallenTeamMate[playerNum - 1] = fallen

            if not is_self and not fallen:
                ballPos = (ballX, ballY, 0)
                if timeBallLastSeen > self.world_model.last_ball_sighting_time:
                    self.world_model.last_ball_sighting_time = timeBallLastSeen
                    self.world_model.last_ball_seen_position = ballPos
                    self.world_model.myVisionPositions[0] = ballPos

                teammatePos = (playerX, playerY)
                self.world_model.myVisionPositions[playerNum] = teammatePos

                # print(playerNum)
                # print("RECIEVED", round(playerX,2), round(playerY,2))

            '''
            print("Data Received:")
            print("\tUNum: {}".format(playerNum))
            print("\tBall last seen time: {}".format(timeBallLastSeen))
            print("\tFallen: {}".format(fallen))
            print("\tBall x: {}".format(ballX))
            print("\tBall y: {}".format(ballY))
            print("\tPlayer x: {}".format(playerX))
            print("\tPlayer y: {}".format(playerY))
