...
class SoccerBehavior(BaseBehavior):
    ...
    def process_hear(self):
        ...
        if heard_from != None:
            self.player_positions[heard_data.heard_from] = (heard_data.my_x, heard_data.my_y)
        ...
        if closest_player_to_ball != None:
            ...
            if self.world_model.playmode == PlayModes.PLAY_ON:
                if self.world_model.my_number == closest_player_to_ball:
                    ...
                    self.pfs_walk.walk_to(PLAYON_POSITIONS[self.world_model.my_number])
            else:
                self.pfs_walk.walk_to(BEFORE_KICKOFF_FORMATION[self.world_model.my_number])
